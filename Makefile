...
.PHONY: setup setup-all install-global demo test test-cov lint render build-latex-image mcp-serve mcp-install-global sync-global serve clean help
...
help:
	...
	@echo "  build-latex-image    Build local/pdflatex Docker image (run once)"
...
render: $(OUT)/resume.tex
	@if command -v pdflatex >/dev/null 2>&1; then \
		...
		MSYS_NO_PATHCONV=1 docker run --rm \
			-v "$(shell pwd -W)/$(OUT):/data" \
			local/pdflatex \
			pdflatex -interaction=nonstopmode -output-directory /data /data/resume.tex; \
	fi
	@echo "PDF written to $(OUT)/resume.pdf"

build-latex-image:
	docker build -t local/pdflatex -f web_app/backend/Dockerfile.latex .
