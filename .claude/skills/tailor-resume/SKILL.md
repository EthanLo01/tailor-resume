...
- **STAR compliance required on every bullet:** Action + Result minimum. Situation and Task are embedded in the role header, not stated in the bullet (compression).
...
5. **Compile to PDF** — after writing `out/<name>.tex`, run:
   ```bash
   python scripts/pdf_compiler.py out/<name>.tex
   ```
   `pdf_compiler.py` tries local `pdflatex` first; if not found, falls back to the `local/pdflatex` Docker image automatically. The PDF is written alongside the `.tex` file. If Docker is not running or the image is missing, report the error and show the manual command from `scripts/pdf_export.md`.
